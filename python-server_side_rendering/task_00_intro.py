"""
    ...
    Returns:
        An invitation, written in an output file, replacing
        each placeholder from the template.
"""
...
def generate_invitations(template, attendees):
    ...
    """
        Iterates over the list of attendees and replaces the placeholders
    ...
    for index, attendee in enumerate(attendees, start=1):
        ...
        if os.path.exists(output_filename):
            print(
                f"Error: The file {output_filename} already exists."
            )
            continue
        ...
        try:
            ...
            print(
                f"Error: Cannot write to file {output_filename}. "
                f"Exception: {e}"
            )
